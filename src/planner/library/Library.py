...
class TemplateLibrary:
    def __init__(self):
        ...
        self.planning_generic = GenericBuilder('builder', {'build'}, ['build', 'templates', 'vars', 'nodes'])
        ...
        self.runtime_generic = GenericBuilder('builder', {'build'}, ['build', 'templates', 'vars', 'nodes'])
    ...
    def compile_node_for_runtime(self, _type, _args):
        ...
        _nodes['nodes'][_id].pop('root')
        return _id, _nodes



    def load_templates(self, templates_file):
        task = Task(message=templates_file.copy(), sender_name='anonymous', keywords={'build'})
            ...
            for args_instance in itertools.product(*args_domains):
                ...
                for postcondition in instance['nodes']['id']['postconditions']:
                    ...
                    for var, val in postcondition.items():
                        self.add_effect(var, val, prob, (template, args_instance))
    ...
    def build_and_save_template_with_args(self, _type, _args, _id=None):
        if id is None:
            _id = self.get_next_uuid(for_planning=True)
        self.templates[self.get_yaml_def(_id, _type, _args)] = (self.compile_node_for_planning(_id, _type, _args), _type, _args)

    def bucketize_from(self, initial_state, subset_of_templates=None):
        return [(tbt.tick(with_memory=copy.deepcopy(initial_state)), t_type, t_args) for tbt, t_type, t_args in subset_of_templates or self.templates]
    ...
    def get_candidate_actions(self, condition, history, state):
        ...
        for pvar, pvals in self.postconditions.items():
            for pval, (prob, (template, args)) in pvals.items():
                # TRICK!
                var = copy.deepcopy(condition['var'])
                var.replace(pvar, pval)
                state.exec({
                    'S': var + ' == ' + val
                })
    ...
    def get_best_templates_for_condition(self, condition, history, state):
        """
    ...
    def check_if_action_threats_condition(self, action, _condition):
        ...
        ctx = {condition['var']: condition['val']}
        exec(action, ctx)
